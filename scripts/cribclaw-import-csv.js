...
import { initDatabase, insertBabyEvent, getAllChats } from '../src/db.js';
// ---------------------------------------------------------------------------
// CSV parser (no deps)
// ---------------------------------------------------------------------------
...
function parseFlexibleDate(raw) {
    ...
    const usMatch = raw.match(/(\d{1,2})\/(\d{1,2})\/(\d{2,4})\s+(\d{1,2}):(\d{2})(?::(\d{2}))?\s*(am|pm)?/i);
    if (usMatch) {
        ...
        if (meridian?.toLowerCase() === 'am' && h === 12)
            h = 0;
        const d2 = new Date(parseInt(year, 10), parseInt(month, 10) - 1, parseInt(day, 10), h, parseInt(minutes, 10), parseInt(seconds || '0', 10));
        if (!isNaN(d2.getTime()))
...
function mapEventType(rawType) {
    ...
    if (/^(feed|feeding|bottle|nurse|nursing|breast|formula)/.test(lower))
        return 'feed';
    if (/^(diaper|nappy)/.test(lower))
        return 'diaper';
    if (/^(sleep|nap)/.test(lower))
        return 'sleep_start'; // will produce sleep_end too
    if (/^(pump|express)/.test(lower))
        return 'pump';
    if (/^(tummy|tummy time)/.test(lower))
        return 'tummy_time';
    if (/^(bath|wash)/.test(lower))
        return 'bath';
    if (/^(solid|puree|cereal|baby food)/.test(lower))
        return 'solids';
    if (/^(growth|weight|height|measure)/.test(lower))
        return 'growth';
    if (/^(milestone)/.test(lower))
        return 'milestone';
    return 'note';
}
function parseDurationMinutes(raw) {
    ...
    if (!isNaN(plain) && plain > 0 && plain < 1440)
...
function parseAmount(raw) {
    ...
    if (mlMatch) {
        ...
        if (!result.amount_oz)
            result.amount_oz = parseFloat((result.amount_ml / 29.5735).toFixed(2));
    }
...
function mapHuckleberry(row) {
    ...
    const attributes = { import_source: 'huckleberry' };
    if (side)
        attributes.side = side.toLowerCase();
    if (condition)
    ...
    if (durationMin)
        attributes.duration_minutes = durationMin;
    if (eventType === 'feed') {
        if (amount)
            Object.assign(attributes, parseAmount(amount));
        if (condition)
            attributes.feed_type = condition.toLowerCase();
        if (side)
            attributes.feed_method = side.toLowerCase();
    }
    if (eventType === 'diaper') {
        // Parse Huckleberry diaper notes like "Both, pee:medium poo:large"
        const diaperInfo = (notes || amount || '').toLowerCase();
        if (/pee|wet|urine/.test(diaperInfo))
            attributes.wet = true;
        if (/poo|dirty|bm|stool/.test(diaperInfo))
            attributes.dirty = true;
        if (/both/.test(diaperInfo)) {
            attributes.wet = true;
            attributes.dirty = true;
        }
    }
...
function mapGlow(row) {
    ...
    const attributes = { import_source: 'glow' };
    if (notes)
        attributes.notes = notes;
    const durationMin = parseDurationMinutes(duration);
    if (durationMin)
...
function mapGeneric(row) {
    ...
    const attributes = { import_source: 'generic' };
    if (notesVal)
        attributes.notes = notesVal;
    ...
    for (const [key, value] of Object.entries(row)) {
        ...
        if (/duration|length|minutes/.test(lower) && value) {
            ...
            if (dur)
                attributes.duration_minutes = dur;
        }
...
function parseArgs(argv) {
    ...
    return { csvFile, chatJid, sender, senderName, apply };
}
function main() {
    ...
    if (!args.csvFile) {
        ...
        console.error('  --apply            Actually write to DB (default: dry run)');
        console.error('');
 ...
 */
export function importCsvFile(csvFilePath, chatJid, senderName = 'CSV Import') {
...
//# sourceMappingURL=cribclaw-import-csv.js.map