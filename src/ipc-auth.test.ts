...
describe('register_group authorization', () => {
  it('non-main group cannot register a group', async () => {
    ...
    expect(groups['tg:-100new']).toBeUndefined();
  });
});
...
describe('IPC message authorization', () => {
  ...
  it('main group can send to any group', () => {
    expect(isMessageAuthorized('main', true, 'tg:-100other', groups)).toBe(true);
    expect(isMessageAuthorized('main', true, 'tg:-100third', groups)).toBe(true);
  });

  it('non-main group can send to its own chat', () => {
    expect(isMessageAuthorized('other-group', false, 'tg:-100other', groups)).toBe(true);
  });

  it('non-main group cannot send to another groups chat', () => {
    expect(isMessageAuthorized('other-group', false, 'tg:-100main', groups)).toBe(false);
    expect(isMessageAuthorized('other-group', false, 'tg:-100third', groups)).toBe(false);
  });

  it('non-main group cannot send to unregistered JID', () => {
    expect(isMessageAuthorized('other-group', false, 'tg:-100unknown', groups)).toBe(false);
  });

  it('main group can send to unregistered JID', () => {
    // Main is always authorized regardless of target
    expect(isMessageAuthorized('main', true, 'tg:-100unknown', groups)).toBe(true);
  });
});
...
describe('register_group success', () => {
  it('main group can register a new group', async () => {
    ...
    const group = getRegisteredGroup('tg:-100new');
  ...
  it('register_group rejects request with missing fields', async () => {
    ...
    expect(getRegisteredGroup('tg:-100partial')).toBeUndefined();
  });
});
